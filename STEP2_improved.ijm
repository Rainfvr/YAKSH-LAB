...
testfolder = getDirectory("Select Project Folder");
...
for (i = 0; i < lengthOf(files); i++) {
  ...
  //print out current status of batch	
	...
	open(yapic_bv + files[i]); 
	raw = getTitle();
	start_time_image = getTime();
	run("8-bit");
	saveAs("Tiff",yapic_bv + raw);
  ...
  //create variable for identifying which yapic out file to open as well 	
	...
	run("Clear Results");
	Ext.CLIJ2_statisticsOfLabelledPixels(raw, yapbv_bin);
	saveAs("results", results_save + "BV_Volume_" + name+ ".csv");
	run("Clear Results");
...
//OPEN UP YAPIC OUTPUT AND SPLIT INTO PERIVASCULAR AND NON PERIVASCULAR MAPS	
	...
	yap = getTitle();
	run("8-bit");
	saveAs("Tiff",yapic + yap);
	
  // push mac_output images to GPU
	Ext.CLIJ2_push(yap);
	
  //threshold macrophages from yapic out	
	Ext.CLIJ2_thresholdOtsu(yap, yap_bin);
	Ext.CLIJ2_watershed(yap_bin,water);
	Ext.CLIJ2_pull(water);
	Ext.CLIJ2_connectedComponentsLabelingDiamond(water, labelmap);
	Ext.CLIJ2_pull(yap_bin);
	yap_bin = getTitle();
	...
	Ext.CLIJ2_push(yap_bin);
	Ext.CLIJ2_push(yapbv_bin);
  ...
  // push pixel count for size filter to GPU
	...
	Ext.CLIJ2_excludeLabelsWithValuesWithinRange(PIXEL_COUNT, labelmap, filtered_labelmap1, 0, 1500);
	run("Clear Results");
	rad = 6;
	Ext.CLIJx_extendLabelsWithMaximumRadius(filtered_labelmap1, labelmap3, rad);
	Ext.CLIJ2_mask(labelmap3,yap_bin,filtered_labelmap)
	Ext.CLIJ2_statisticsOfBackgroundAndLabelledPixels(yapbv_bin, filtered_labelmap);
	Ext.CLIJ2_pull(filtered_labelmap);
